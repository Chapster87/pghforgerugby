...
<div class="card">
    <div class="card-body">
        <div class="sp-template sp-template-countdown">
            ...
            if(!empty($matchupData) && $matchupData['isForgeHome']) {
                echo '<span class="h5 countdown-home-away">Home</span><hr/>';
            } else {
                echo '<span class="h5 countdown-home-away">Away</span><hr/>';
            }
            ...
            ?>
                <div class="event-name sp-event-name">
                    <?php
                    if ( $matchupData && $show_logos ) {
                        if (has_post_thumbnail($matchupData['homeTeamID'])) {
                            echo '<div class="home-team-logo">';

                                if ( $link_teams ) {
                                    echo '<a class="team-logo" href="' . esc_url( get_post_permalink($matchupData['homeTeamID'])) . '" title="' . $matchupData['homeTeamName'] . '">' . get_the_post_thumbnail($matchupData['homeTeamID'], 'sportspress-fit-icon') . '</a>';
                                } else {
                                    echo get_the_post_thumbnail($matchupData['homeTeamID'], 'sportspress-fit-icon', array('class' => 'team-logo'));
                                }
                        ...
                        <div class="event-teams">
                            <?php // Team names swapped with css to sync with against symbol ?>
                            <div class="event-team home-team <?php if ($matchupData['isForgeHome']) : echo 'forge'; else : echo 'not-forge'; endif; ?>"><?php echo $matchupData['homeTeamNameShort']; ?></div>
                            <?php
                                if ($matchupData['isForgeHome']) {
                                    echo '<div class="against versus">vs</div>';
                                } else {
                                    echo '<div class="against at">@</div>';
                                }
                            ?>
                            <div class="event-team away-team <?php if ($matchupData['isForgeAway']) : echo 'forge'; else : echo 'not-forge'; endif; ?>"><?php echo $matchupData['awayTeamNameShort']; ?></div>
                        </div>

                        <?php
                        if (has_post_thumbnail($matchupData['awayTeamID'])) {
                            echo '<div class="away-team-logo">';
                            if ( $link_teams ) {
                                echo '<a class="team-logo" href="' . esc_url( get_post_permalink($matchupData[' ID'])) . '" title="' . $matchupData['awayTeamName'] . '">' . get_the_post_thumbnail($matchupData['awayTeamID'], 'sportspress-fit-icon') . '</a>';
                            } else {
                                echo get_the_post_thumbnail($matchupData['awayTeamID'], 'sportspress-fit-icon', array('class' => 'team-logo'));
                            }
                            echo '</div>';
                        }
                    }
                ...
                <p class="countdown-dev long-countdown">
                    <time datetime="">
